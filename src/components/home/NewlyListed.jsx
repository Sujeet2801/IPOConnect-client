...
function NewlyListedIPO() {
  ...
  return (
    <section className="bg-gradient-to-b from-green-400 via-green-500 to-green-600 
    px-6 py-8 rounded-xl shadow-lg text-white">
      {/* Section Banner */}
      <div className="text-center mb-8">
        <h1 className="text-4xl font-extrabold tracking-tight drop-shadow-md">🚀 Newly Listed IPOs</h1>
        <p className="mt-2 text-lg opacity-90 max-w-2xl mx-auto">
          Check out the most recent IPO listings, now live on the market. Explore their 
          prices, opening dates, and other crucial details.
        </p>
        <div className="mt-4 flex flex-wrap justify-center gap-4 text-sm">
          <span className="bg-white/20 px-3 py-1 rounded-full">Live Listings</span>
          <span className="bg-white/20 px-3 py-1 rounded-full">Market Updates</span>
          <span className="bg-white/20 px-3 py-1 rounded-full">Invest Now</span>
        </div>
      </div>

      {/* Header and CTA */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
        <h2 className="text-2xl font-semibold">Top Newly Listed IPOs</h2>
        <button
          onClick={() => nav("/Listed")}
          className="mt-3 sm:mt-0 bg-white text-green-600 font-semibold px-4 py-2
      ...
      {loading ? (
        ...
        <div className="flex flex-col md:flex-row gap-6">
          {ipos.map((ipo, index) => (
            <div
              key={index}
              className="bg-white text-gray-800 p-5 rounded-xl shadow-md w-full
              md:w-1/3 hover:shadow-xl transition-transform hover:-translate-y-1"
            >
              <p className="text-center font-bold text-green-600 text-lg">{ipo.companyName}</p>
