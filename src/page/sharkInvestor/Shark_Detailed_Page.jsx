...
const SharkDetailedPage = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-tr from-blue-50 via-white to-purple-50 py-8 
    px-6 lg:px-20">
      {/* Profile Card */}
      <div className="bg-white shadow-2xl rounded-3xl p-8 md:p-12 flex flex-col md:flex-row 
      items-center gap-10">
        <img
          src={shark.imageUrl}
          alt={shark.name}
        ...
        <div className="text-center md:text-left">
          <h1 className="text-4xl font-extrabold text-purple-800">{shark.name}</h1>
          ...
          <div className="mt-4 flex flex-col sm:flex-row gap-5 text-lg text-gray-700 font-medium">
            <div className="bg-green-100 text-green-800 px-4 py-2 rounded-full">
              💰 Net Worth: <span className="font-bold">{shark.netWorth} Cr</span>
            </div>
            <div className="bg-blue-100 text-blue-800 px-4 py-2 rounded-full">
              📁 Portfolio: <span className="font-bold">{shark.portfolio}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mt-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        ...
        <InfoCard
          title="🚀 Stage Focus"
          items={Object.keys(shark.stageFocus || {}).filter(stage => shark.stageFocus[stage])}
          bg="bg-blue-50"
        />
        <InfoCard
